...
export function createSegmenter(options = {}) {
  const {
    movementThreshold = 0.05, // avg per-landmark displacement per frame
  ...
  function computeCentroid(handsInFrame) {
  ...
  /**
   * Call once per detected frame with the raw hands-in-frame array (same
   * shape MediaPipe returns) and the current timestamp in ms.
   *
   ...
   */
  function pushFrame(handsInFrame, timestampMs) {
    const handCount = handsInFrame.length;
    ...
    const centroid = computeCentroid(handsInFrame);

    if (state === "idle") {
      state = "accumulating";
      bufferFrames = [handsInFrame];
    ...
    bufferFrames.push(handsInFrame);

    const displacement = previousCentroid ? distance(centroid, previousCentroid) : 0;
    previousCentroid = centroid;
