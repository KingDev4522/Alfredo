...
// NOTE: reveals use fromTo + immediateRender:false so copy never gets
// trapped at opacity 0 when a ScrollTrigger fails to fire.
...
export function useInViewReveal(ref, { y = 40, delay = 0, duration = 0.9 } = {}) {
  useEffect(() => {
    ...
    if (reduced) return
    const ctx = gsap.context(() => {
      // fromTo + immediateRender:false keeps copy visible if the trigger
      // never fires (Lenis/pin layout shifts) - it only hides at play time.
      gsap.fromTo(
        el,
