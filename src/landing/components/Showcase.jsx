...
function Frame({ src, onLoad }) {
  const fix = REFRAME[src]
  return (
    // aspect on the wrapper reserves layout before the JPG loads, so the
    // scrub distance (scrollWidth) is correct from the first measure.
    <div className="pointer-events-none relative aspect-[8/5] h-[190px] shrink-0 select-none overflow-hidden bg-neutral-950 sm:h-[240px] md:h-[300px]">
      <img
        src={src}
        alt=""
        aria-hidden="true"
        ...
        className="h-full w-full object-cover contrast-[1.05] brightness-[0.96] saturate-[1.05]"
      />
    </div>
  )
}

function Strip({ images, className = '', onFrameLoad }) {
  // doubled for a seamless travel distance
  const loop = [...images, ...images]
  return (
    <div className={`flex w-max gap-3 md:gap-4 ${className}`} aria-hidden={false}>
      {loop.map((src, i) => (
        <Frame key={`${src}-${i}`} src={src} onLoad={onFrameLoad} />
      ))}
    </div>
  )
}
...
export default function Showcase() {
  ...
  // Late image loads change track widths - re-measure so rows stay
  // full-bleed and triggers fire at the right scroll positions.
  const onFrameLoad = useCallback(() => {
    loadedRef.current += 1
    const n = loadedRef.current
    if (n === 1 || n % 6 === 0) ScrollTrigger.refresh()
  }, [])
    ...
    <section id="showcase" ref={rootRef} aria-label="Field frames" className="relative overflow-hidden bg-black py-24 md:py-32">
      ...
      <div className="relative">
        <div className="flex flex-col gap-3 md:gap-4">
          <div className="show-track will-change-transform"><Strip images={ROWS[0]} onFrameLoad={onFrameLoad} /></div>
          <div className="show-track will-change-transform"><Strip images={ROWS[1]} onFrameLoad={onFrameLoad} /></div>
          <div className="show-track will-change-transform"><Strip images={ROWS[2]} onFrameLoad={onFrameLoad} /></div>
        </div>
        ...
        <div className="show-statement pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
          <img
            ...
            height="72"
            draggable={false}
            className="mb-5 block h-16 w-16 border-0 bg-transparent object-contain outline-none md:h-[72px] md:w-[72px]"
          />
